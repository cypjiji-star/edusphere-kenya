...
const enrolmentSchema = z.object({
  ...
  boardingStatus: z.string({
    required_error: "Please select a boarding status.",
  }),
  ...
  admissionYear: z.string({
    required_error: "Please select an admission year.",
  }),
...
export default function StudentEnrolmentPage() {
  ...
  const [bulkEnrolmentFile, setBulkEnrolmentFile] = React.useState<File | null>(
    null,
  );
  ...
  const [recentEnrolments, setRecentEnrolments] = React.useState<
    RecentEnrolment[]
  >([]);
  ...
  React.useEffect(() => {
    ...
    const unsubClasses = onSnapshot(classesQuery, (snapshot) => {
      const options = snapshot.docs.map((doc) => ({
        value: doc.id,
        label: `${doc.data().name} ${doc.data().stream || ""}`.trim(),
      }));
    ...
    reader.onload = (e) => {
      try {
        ...
        toast({
          ...
          description: "The file format might be incorrect.",
          variant: "destructive",
        });
      } finally {
  ...
  const generateAdmissionNumber = async (
    ...
    const counterRef = doc(
      firestore,
      `schools/${schoolId}/counters/student_admissions`,
    );
    let nextNumber = 1;
  ...
  async function onSubmit(values: EnrolmentFormValues) {
    if (!schoolId || !adminUser) {
      toast({
        title: "Error",
        description: "School ID or Admin user is missing.",
        variant: "destructive",
      });
      return;
    }
    ...
    try {
      ...
      const parentQuery = query(
        collection(firestore, `schools/${schoolId}/users`),
        where("email", "==", values.parentEmail),
        where("role", "==", "Parent"),
      );
      ...
      await runTransaction(firestore, async (transaction) => {
        const admissionNumber =
          ...
          (await generateAdmissionNumber(schoolId, values.admissionYear));
        const admissionDocUrls = await Promise.all(
          admissionDocs.map(async (file) => {
            const storageRef = ref(
              storage,
              `${schoolId}/admission_docs/${Date.now()}_${file.name}`,
            );
        ...
        const classTeacherId = classSnap.exists()
          ? classSnap.data().teacherId
          : null;

        let totalFee = 0;
        if (values.generateInvoice) {
          const feeStructureRef = doc(
            firestore,
        ...
        const studentUserDocRef = doc(
          collection(firestore, "schools", schoolId, "users"),
        );
        transaction.set(studentUserDocRef, {
          id: studentUserDocRef.id,
          schoolId,
          role: "Student",
          status: "Approved",
          ...
          class:
            classOptions.find((c) => c.value === values.classId)?.label ||
            "N/A",
        ...
        if (values.generateInvoice && totalFee > 0) {
          const chargeTransactionRef = doc(
            collection(studentUserDocRef, "transactions"),
          );
        ...
        if (amountPaid > 0) {
          const paymentTransactionRef = doc(
            collection(studentUserDocRef, "transactions"),
          );
          transaction.set(paymentTransactionRef, {
            date: serverTimestamp(),
            description: "Initial Fee Payment on Enrolment",
            type: "Payment",
            amount: -amountPaid,
            balance: initialBalance,
          });
          ...
          transaction.set(schoolTransactionRef, {
            studentId: studentUserDocRef.id,
      ...
      if (error.code === "auth/email-already-in-use")
        errorMessage =
          "This parent email is already registered. Please use a different email or log in to their existing account to add another child.";
  ...
  const handleSaveDraft = () => {
    toast({
      title: "Draft Saved",
      description:
        "The student enrolment application has been saved as a draft.",
    });
  };
                    ...
                    <FormField
                      ...
                      render={({ field }) => (
                        <FormItem>
                          ...
                          <FormControl>
                            <Input placeholder="e.g., SCH-1234" {...field} />
                          </FormControl>
                          <FormDescription>
                ...
                <CardContent className="space-y-6">
                    ...
                    <FormField
                      ...
                      render={({ field }) => (
                        <FormItem>
                          <FormLabel>First Name</FormLabel>
                          <FormControl>
                            <Input placeholder="e.g., Mark" {...field} />
                          </FormControl>
                          <FormMessage />
                        </FormItem>
                      )}
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                    <FormField
                      ...
                      render={({ field }) => (
                        <FormItem>
                          <FormLabel>Parent's Login Email</FormLabel>
                          <FormControl>
                            <Input
                              type="email"
                    ...
                    <FormField
                      ...
                      render={({ field }) => (
                        <FormItem>
                          ...
                          <FormDescription>
                            Only required for new parent accounts.
                          </FormDescription>
                          <FormMessage />
                        </FormItem>
                      )}
                  ...
                  <FormField
                    ...
                    render={({ field }) => (
                      <FormItem>
                        ...
                        <FormControl>
                          <Textarea
                            placeholder="e.g., Peanuts, Pollen, Lactose Intolerance. If none, write 'None'."
                            {...field}
                          />
                        </FormControl>
                        <FormMessage />
                  ...
                  <FormField
                    ...
                    render={({ field }) => (
                      <FormItem>
                        ...
                        <FormControl>
                          <Textarea
                            placeholder="e.g., Asthma, Diabetes. If none, write 'None'."
                            {...field}
                          />
                        </FormControl>
                        <FormMessage />
              ...
              <Card>
                ...
                <CardContent className="space-y-6">
                  ...
                  <FormField
                    ...
                    render={({ field }) => (
                      <FormItem>
                        <FormLabel>Initial Fee Payment (KES)</FormLabel>
                  ...
                  <div className="space-y-2">
                    ...
                    {admissionDocs.length > 0 ? (
                      <div className="space-y-2">
                        ...
                        <Label
                          htmlFor="dropzone-file-docs-more"
                          className="w-full"
                        >
                          <Button
                            type="button"
                    ...
                    render={({ field }) => (
                      <FormItem className="flex flex-row items-center justify-between rounded-lg border p-3 shadow-sm">
                        <div className="space-y-0.5">
                          <FormLabel>Generate Pro-forma Invoice</FormLabel>
                        </div>
                        <FormControl>
                          <Switch
                            checked={field.value}
                        ...
                        {recentEnrolments.length > 0 ? (
                          recentEnrolments.map((enrolment) => (
                            <TableRow key={enrolment.id}>
                              ...
                              <TableCell>
                                {getStatusBadge(enrolment.status)}
                              </TableCell>
                            </TableRow>
                          ))
                        ) : (
