...
export async function publishGradesAction(schoolId: string, examId: string) {
  ...
  try {
    ...
    if (gradesSnapshot.empty) {
      return {
        success: true,
        message: "No pending grades found to publish for this exam.",
      };
    }
    ...
    return {
      success: true,
      message: `Successfully published ${gradesSnapshot.size} grades.`,
    };
  } catch (error) {
    console.error("Error publishing grades:", error);
    return {
      success: false,
      message: "An error occurred while publishing grades.",
    };
  }
}
