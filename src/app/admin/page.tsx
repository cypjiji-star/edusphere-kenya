...
export default function AdminDashboard() {
  ...
  React.useEffect(() => {
    ...
    unsubscribers.push(
      onSnapshot(usersQuery, (usersSnapshot) => {
        const studentCount = usersSnapshot.docs.filter(
          (doc) => doc.data().role === "Student",
        ).length;
        setStats((prev) => ({
          ...prev,
          totalStudents: studentCount,
          totalStaff: usersSnapshot.size - studentCount,
        }));
  ...
  const overviewStats = [
    ...
    {
      title: "Overall Fee Balance",
      stat: formatCurrency(stats.overallFeeBalance),
      ...
      <div className="flex h-screen items-center justify-center">
        <div className="text-center">
          ...
          <p className="text-muted-foreground">
            Please select a school from the developer portal.
          </p>
          ...
          <div className="grid gap-6 sm:grid-cols-2 lg:grid-cols-4">
            {isLoading
              ...
              : overviewStats.map((stat) => (
                  <Link
                    href={
                      `${overviewLinks[stat.title]}?schoolId=${schoolId}` || "#"
                    }
                    key={stat.title}
                  >
          ...
          <CardContent className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-4">
            ...
            <Button asChild size="lg">
              <Link href={`/admin/fees?schoolId=${schoolId}`}>
                <CircleDollarSign className="mr-2" /> Record Bulk Payment
              </Link>
            </Button>
            <Button asChild size="lg">
              <Link href={`/admin/announcements?schoolId=${schoolId}`}>
                <Megaphone className="mr-2" /> Send Announcement
              </Link>
            </Button>
          </CardContent>
        </Card>
        ...
        <div className="lg:col-span-3">
          <Card>
            <CardHeader>
              ...
              <CardDescription>
                A live feed of important events across the school.
              </CardDescription>
            </CardHeader>
            <CardContent>
              {isLoading ? (
                ...
                <ScrollArea className="h-[500px] pr-4">
                  <div className="space-y-6">
                    {activities.map((activity, index) => (
                      <Link
                        key={activity.id}
                        href={`${activity.href}?schoolId=${schoolId}` || "#"}
                        ...
                        <div className="flex items-start gap-4">
                          ...
                          <div className="flex-1">
                            <p className="text-sm font-medium">
                              <span className="bg-muted dark:bg-transparent px-2 py-1 rounded-md">
                                {activity.title}
                              </span>
                            </p>
