...
import { ShieldAlert, AlertTriangle, ArrowRight, Loader2, UserX, Clock } from 'lucide-react';
...
export function SecurityAlertsWidget({ schoolId }: { schoolId: string }) {
        ...
        const unsubscribe = onSnapshot(logsQuery, (snapshot) => {
            ...
            snapshot.forEach(doc => {
                ...
                if (log.action === 'USER_LOGIN_FAILURE') {
                    ...
                    failedLoginAttempts[userIdentifier] = (failedLoginAttempts[userIdentifier] || 0) + 1;
                }
    ...
    const getAlertIcon = (type: SecurityAlert['type']) => {
        switch(type) {
            case 'Failed Logins': return <UserX className="h-5 w-5 text-red-500" />;
            case 'After-Hours Activity': return <Clock className="h-5 w-5 text-yellow-500" />;
            case 'Sensitive Change': return <AlertTriangle className="h-5 w-5 text-orange-500" />;
  ...
  return (
    <Card>
      <CardHeader>
        ...
        <CardDescription>A summary of recent, potentially sensitive activity.</CardDescription>
      </CardHeader>
      <CardContent>
         {isLoading ? (
