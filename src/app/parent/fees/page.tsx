  ...
  React.useEffect(() => {
    ...
    const unsubStudent = onSnapshot(studentDocRef, (studentSnap) => {
      if (studentSnap.exists()) {
        ...
        const balance =
          (studentData.totalFee || 0) - (studentData.amountPaid || 0);
        const dueDate =
          studentData.dueDate instanceof Timestamp
            ? studentData.dueDate.toDate()
            : studentData.dueDate
              ? new Date(studentData.dueDate)
              : new Date();
    ...
    const unsubTransactions = onSnapshot(transactionsQuery, (snapshot) => {
      const fetchedLedger = snapshot.docs.map(
    ...
    <div className="p-4 sm:p-6 lg:p-8 space-y-6">
      <div className="mb-2 p-4 md:p-6 bg-card border rounded-lg">
        ...
        <p className="text-muted-foreground">
          View fee statements, balances, and payment information.
        </p>
      </div>
      ...
      {isOverdue && (
        <Alert variant="destructive">
          ...
          <AlertDescription>
            The fee balance for this term is overdue. Please make a payment as
            soon as possible.
          </AlertDescription>
        </Alert>
      )}
          ...
          <div className="grid gap-6 md:grid-cols-2 lg:grid-cols-4">
            <Card>
              <CardHeader className="pb-2">
                ...
                <CardTitle
                  className={`text-4xl ${feeSummary.balance > 0 ? "text-destructive" : "text-green-600"}`}
                >
              ...
              <CardContent>
                <div className="text-xs text-muted-foreground">
                  Due:{" "}
                  {new Date(feeSummary.dueDate).toLocaleDateString("en-GB")}
                </div>
              </CardContent>
            </Card>
            <Card>
              <CardHeader className="pb-2">
                ...
                <CardTitle className="text-2xl">
                  {formatCurrency(feeSummary.totalBilled)}
                </CardTitle>
              </CardHeader>
            </Card>
            <Card>
              ...
              <CardHeader>
                <div className="flex items-center justify-between">
                  ...
                  <DropdownMenu>
                    ...
                    <DropdownMenuContent>
                      <DropdownMenuItem>
                        <Printer className="mr-2" /> Print Statement
                      </DropdownMenuItem>
                      <DropdownMenuItem>
                ...
                <CardDescription>
                  A detailed transaction history for the current term.
                </CardDescription>
              </CardHeader>
              <CardContent>
                    ...
                    <TableBody>
                      {ledger.map((item) => (
                        <TableRow key={item.id}>
                          ...
                          <TableCell
                            className={`text-right ${item.amount > 0 ? "text-destructive" : ""}`}
                          >
                            {item.amount > 0
                      ...
                      {ledger.length === 0 && (
                        <TableRow>
                          <TableCell colSpan={5} className="h-24 text-center">
                            No transactions for this term yet.
                          </TableCell>
                        </TableRow>
                      )}
                    </TableBody>
              ...
              <CardContent className="space-y-6">
                ...
                <div className="space-y-4">
                  ...
                  <div className="text-xs space-y-1 text-muted-foreground bg-muted/50 p-3 rounded-md">
                    ...
                    <p>
                      Account: <span className="font-bold">1122334455</span>
                    </p>
                    <p>
                      Branch: <span className="font-bold">University Way</span>
                    </p>
                    <p>
                      Account Name:{" "}
                      <span className="font-bold">EduSphere High School</span>
                    </p>
                  </div>
        ...
        <TabsContent value="history" className="mt-6">
          <Card>
            <CardHeader>
              ...
              <CardDescription>
                A record of all payments made for{" "}
