...
export function ParentChatLayout() {
  ...
  React.useEffect(() => {
    ...
    // Find the conversation between the parent and the admin support.
                ...
                {messages.map((msg, index) => {
                  ...
                  return (
                    <div key={msg.id || index} className={cn("flex items-end gap-2", isUserSender ? 'justify-end' : 'justify-start')}>
                      ...
                      {isUserSender && (<Avatar className="h-8 w-8"><AvatarImage src={user?.photoURL || "https://picsum.photos/seed/parent-avatar/100"} /><AvatarFallback>{user?.displayName?.charAt(0) || 'P'}</AvatarFallback></Avatar>)}
                    </div>
                  );
                })}
