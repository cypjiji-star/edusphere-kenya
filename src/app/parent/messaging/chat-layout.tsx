...
export function ParentChatLayout() {
  ...
  React.useEffect(() => {
    ...
    const fetchContacts = async () => {
      ...
      const childrenQuery = query(collection(firestore, `schools/${schoolId}/students`), where('parentId', '==', user.uid));
      ...
      const teachersQuery = query(collection(firestore, `schools/${schoolId}/users`), where('__name__', 'in', Array.from(teacherIds)));
      const teachersSnapshot = await getDocs(teachersQuery);
      
      const teacherContacts = teachersSnapshot.docs.map(doc => {
          const data = doc.data();
          return {
              id: doc.id,
      ...
      setNewContactOptions(teacherContacts);
    };
          ...
          <div className="flex-shrink-0 p-4 border-b">
            <div className="flex items-center justify-between">
              ...
              <Dialog>
                ...
                <DialogContent>
                  <DialogHeader><DialogTitle>New Conversation</DialogTitle><DialogDescription>Select a teacher to start a new chat.</DialogDescription></DialogHeader>
                  <div className="py-4">
                    <Label htmlFor="new-chat-recipient">Recipient</Label>
                    <Select onValueChange={handleCreateConversation}>
                      <SelectTrigger id="new-chat-recipient"><SelectValue placeholder="Select a teacher..." /></SelectTrigger>
                      <SelectContent>
            ...
            <div className="relative mt-4">
              ...
              <Input placeholder="Search messages or teachers..." className="pl-8" value={searchTerm} onChange={(e) => setSearchTerm(e.target.value)} />
            </div>
          </div>
              ...
              <div className="flex-1 overflow-y-auto p-4 space-y-4">
                {(messages[selectedConvo.id] || []).map((msg, index) => (
                  <div key={msg.id || index} className={cn("flex items-end gap-2", isSender(msg) ? 'justify-end' : 'justify-start')}>
                    ...
                    {isSender(msg) && (<Avatar className="h-8 w-8"><AvatarImage src={user?.photoURL || "https://picsum.photos/seed/parent-avatar/100"} /><AvatarFallback>{user?.displayName?.charAt(0) || 'P'}</AvatarFallback></Avatar>)}
                  </div>
                ))}
