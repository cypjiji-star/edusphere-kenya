...
export default function ParentDashboard() {
  ...
  React.useEffect(() => {
    ...
    try {
      ...
      unsubscribers.push(
        onSnapshot(childrenQuery, (studentsSnapshot) => {
  ...
  React.useEffect(() => {
    ...
    const unsubAttendance = onSnapshot(
      ...
      (attendanceSnapshot) => {
        ...
        const present = attendanceSnapshot.docs.filter((r) =>
          ["present", "late"].includes(r.data().status.toLowerCase()),
        ).length;
        const attendance =
    ...
    const unsubGrades = onSnapshot(gradesQuery, (gradesSnapshot) => {
      const recentGrades = gradesSnapshot.docs.map((d) => ({
        subject: d.data().subject || "Unknown",
        grade: d.data().grade,
        date: (d.data().date as Timestamp).toDate().toLocaleDateString("en-GB"),
      }));

      // This is a simplified overall grade calculation
      const scores = gradesSnapshot.docs
        .map((d) => parseInt(d.data().grade, 10))
        .filter((s) => !isNaN(s));
      const overallGrade =
        scores.length > 0
      ...
      setSelectedChild((prev) =>
        prev ? { ...prev, recentGrades, overallGrade } : null,
      );
    });
    ...
    return (
      <div className="flex h-[calc(100vh-200px)] items-center justify-center">
        <div className="text-center">
          ...
          <Button asChild>
            <Link href={`/parent/support?schoolId=${schoolId}`}>
              Contact Support
            </Link>
          </Button>
        </div>
    ...
    <div className="p-4 sm:p-6 lg:p-8">
      <Card className="mb-8 overflow-hidden">
        <CardHeader className="bg-muted/30">
          ...
          {clientReady && (
            <CardDescription>
              Your dashboard for the {schoolName} parent portal.
            </CardDescription>
          )}
        </CardHeader>
      ...
      <div className="grid gap-6 sm:grid-cols-2 lg:grid-cols-4 mb-8">
        <Card>
          ...
          <CardContent>
            <div
              className={`text-2xl font-bold ${getAttendanceColor(selectedChild?.attendance || 0)}`}
            >
          ...
          <CardContent>
            ...
            {clientReady && (
              <p className="text-xs text-muted-foreground">
                ...
                {selectedChild
                  ? new Date(
                      selectedChild.feeStatus.dueDate,
      ...
      <div className="grid gap-8 lg:grid-cols-3">
        <div className="lg:col-span-1 space-y-8">
          ...
          {user && (
            <AnnouncementsWidget schoolId={schoolId} currentUserId={user.uid} />
          )}
        </div>
              ...
              <div>
                ...
                <div className="space-y-3">
                  {(selectedChild?.recentGrades || []).map((grade, index) => (
                    <div
                      key={index}
                      className="flex justify-between items-center text-sm p-3 rounded-md bg-muted/50"
                    >
