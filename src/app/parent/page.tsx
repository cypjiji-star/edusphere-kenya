...
export default function ParentDashboard() {
    ...
    const fetchChildrenData = React.useCallback(async () => {
        ...
        try {
            ...
            const fetchedChildren = await Promise.all(
                childrenSnapshot.docs.map(async (studentDoc) => {
                    ...
                    const numericScores = allGradesSnapshot.docs
                        .map(d => {
                            const grade = d.data().grade;
                            // Handle both numeric and letter grades
                            if (typeof grade === 'number') return grade;
                            if (typeof grade === 'string' && !isNaN(Number(grade))) return Number(grade);
                            return 0;
                        })
    ...
    if (error) {
        return (
            <div className="flex h-[calc(100vh-200px)] items-center justify-center">
                <ErrorMessage message={error} onRetry={fetchChildrenData} />
            </div>
        );
    }
