...
export function AssignmentForm() {
  ...
  React.useEffect(() => {
    ...
    const unsubscribe = onSnapshot(q, (snapshot) => {
      const classesData: TeacherClass[] = snapshot.docs.map((d) => ({
        id: d.id,
  ...
  async function onSubmit(values: AssignmentFormValues) {
    if (!schoolId || !user) {
      toast({
        title: "Error",
        description: "Not authenticated or school ID is missing.",
        variant: "destructive",
      });
      return;
    }
    ...
    const className =
      teacherClasses.find((c) => c.id === values.classId)?.name ||
      "Unknown Class";
