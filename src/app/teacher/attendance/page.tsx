    ...
    try {
      if (attachment) {
        const storageRef = ref(
          storage,
          `schools/${schoolId}/leave_attachments/${Date.now()}_${attachment.name}`,
        );
                ...
                <TableBody>
                  {leaveApplications.map((app) => (
                    <TableRow key={app.id}>
                      ...
                      <TableCell>
                        {format(app.startDate.toDate(), "dd/MM/yy")} -{" "}
                        {format(app.endDate.toDate(), "dd/MM/yy")}
                      </TableCell>
...
export default function AttendancePage() {
  ...
  useEffect(() => {
    ...
    const unsub = onSnapshot(
      q,
      (snap) => {
        const classesData: TeacherClass[] = snap.docs.map((d) => ({
          id: d.id,
          name: `${d.data().name} ${d.data().stream || ""}`.trim(),
        }));
  ...
  const fetchAttendanceData = useCallback(async () => {
    ...
    try {
      ...
      toast({
        title: "Error",
        description: "Could not load student attendance.",
        variant: "destructive",
      });
    } finally {
      setIsLoading(false);
  ...
  const studentFilteredRecords = React.useMemo(() => {
    ...
    return allStudentAttendance.filter(
      (record) => record.studentId === selectedStudentForAnalytics.id,
    );
  }, [allStudentAttendance, selectedStudentForAnalytics]);

  const studentSummaryStats = React.useMemo(() => {
    const present = studentFilteredRecords.filter(
      (r) => r.status === "present",
    ).length;
    const absent = studentFilteredRecords.filter(
      (r) => r.status === "absent",
    ).length;
    const late = studentFilteredRecords.filter(
      (r) => r.status === "late",
    ).length;
  ...
  const displayedStudents = allTeacherStudents.filter(
    (s) =>
    ...
    <div className="p-4 sm:p-6 lg:p-8 space-y-4">
      <div className="mb-2">
        ...
        <p className="text-muted-foreground">
          Manage attendance for your classes and view your own record.
        </p>
      </div>
                              ...
                              <TableCell>
                                ...
                                >
                                  <SelectTrigger className="w-36">
                                    <SelectValue>
                                      {getAttendanceBadge(student.status)}
                                    </SelectValue>
                                  </SelectTrigger>
                                  <SelectContent>
                                    <SelectItem value="present">
                                      {getAttendanceBadge("present")}
                                    </SelectItem>
                                    <SelectItem value="absent">
                                      {getAttendanceBadge("absent")}
                                    </SelectItem>
                                    <SelectItem value="late">
                                      {getAttendanceBadge("late")}
                                    </SelectItem>
                                    <SelectItem value="unmarked">
                                      {getAttendanceBadge("unmarked")}
                                    </SelectItem>
                                  </SelectContent>
                                </Select>
                              </TableCell>
        ...
        <TabsContent value="student-analytics" className="mt-4">
          <Card>
            <CardHeader>
              ...
              <CardDescription>
                Search for a student to view their complete attendance history
                for the term.
              </CardDescription>
            </CardHeader>
            <CardContent>
              <Combobox
                options={displayedStudents.map((s) => ({
                  value: s.id,
                  label: s.name,
                }))}
                ...
                onValueChange={(value) =>
                  setSelectedStudentForAnalytics(
                    allTeacherStudents.find((s) => s.id === value) || null,
                  )
                }
                      ...
                      <div className="flex items-center justify-between">
                        <div className="flex items-center gap-4">
                          <Avatar className="h-16 w-16">
                            <AvatarImage
                              src={selectedStudentForAnalytics.avatarUrl}
                            />
                            <AvatarFallback>
                          ...
                          <div>
                            <CardTitle className="text-2xl">
                              {selectedStudentForAnalytics.name}
                            </CardTitle>
                            <CardDescription>
                              Admission No:{" "}
                              {selectedStudentForAnalytics.admissionNumber}
                            </CardDescription>
                          </div>
                        </div>
                        <Button
