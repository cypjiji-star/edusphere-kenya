...
export function MyAttendanceWidget() {
  ...
  const [checkOutTime, setCheckOutTime] = React.useState<Timestamp | null>(
    null,
  );
  ...
  React.useEffect(() => {
    ...
    const attendanceDocRef = doc(
      firestore,
      `schools/${schoolId}/teacher_attendance`,
      `${user.uid}_${todayStr}`,
    );
  ...
  const getStatusInfo = (): {
    ...
    if (status === "Present" && checkInTime) {
      return {
        icon: CheckCircle,
        text: `Checked In at ${format(checkInTime.toDate(), "h:mm a")}`,
          ...
          <>
            <div>
              <Badge
                ...
                className={`text-base ${badgeClass}`}
              >
            ...
            {status === "Pending" && (
              <div>
                ...
                <p className="text-xs text-muted-foreground mt-2">
                  Geofencing enabled: You must be on-site to check in.
                </p>
              </div>
            )}
            {status === "Present" && !checkOutTime && (
              <div>
                ...
                <p className="text-xs text-muted-foreground mt-2">
                  Geofencing enabled: You must be on-site to check out.
                </p>
              </div>
            )}
            {status === "CheckedOut" && checkOutTime && (
              <p className="text-sm text-muted-foreground">
                Checked out at {format(checkOutTime.toDate(), "h:mm a")}.
              </p>
            )}
          </>
        )}
