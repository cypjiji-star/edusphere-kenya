...
export default function SportsPage() {
  ...
  React.useEffect(() => {
    ...
    const teamsQuery = query(
      collection(firestore, "schools", schoolId, "teams"),
    );
    const unsubTeams = onSnapshot(
      teamsQuery,
      (snapshot) => {
        const teamsData = snapshot.docs.map(
          (doc) => ({ id: doc.id, ...doc.data() }) as SportsTeam,
        );
    ...
    const unsubTeachers = onSnapshot(teachersQuery, (snapshot) => {
      const teachers = snapshot.docs.map(
        (doc) =>
          ({
            id: doc.id,
            name: doc.data().name,
  ...
  const sportsTeams = allSportsTeams.filter((team) => {
    ...
    return (
      team.coach === (currentTeacher?.name || user.displayName || user.email)
    );
  });
  ...
  const handleIconGeneration = async () => {
    ...
    try {
      const result = await generateTeamIcon({
      ...
      if (result.icon) {
        ...
        toast({
          ...
          description: `We've selected an icon for your team.`,
        });
      } else {
  ...
  const handleCreateTeam = async () => {
    ...
    try {
      ...
      const selectedTeacher = allTeachers.find(
        (teacher) => teacher.id === newTeamCoach,
      );
      ...
      <div className="flex flex-col items-start gap-4 md:flex-row md:items-center md:justify-between mb-6">
        <div className="text-left">
          ...
          <p className="text-muted-foreground">
            Manage the teams and clubs you are responsible for.
          </p>
        </div>
        <Dialog>
          ...
          <DialogContent>
            ...
            <div className="grid gap-4 py-4">
              ...
              <div className="space-y-2">
                ...
                <Textarea
                  id="team-description"
                  placeholder="e.g., The official school basketball team for inter-school competitions."
                ...
                <Card key={team.id} className="flex flex-col">
                  ...
                  <CardFooter>
                    <Button asChild variant="default" className="w-full">
                      <Link
                        href={`/teacher/sports/${team.id}?schoolId=${schoolId}`}
                      >
                        Manage Team
