...
type TimetableData = Record<
  string,
  Record<string, { subject: { name: string; teacher: string }; room: string }>
>;
type PeriodData = {
  id: number;
  time: string;
...
export function TimetableWidget() {
  ...
  useEffect(() => {
    ...
    const periodsRef = doc(
      firestore,
      `schools/${schoolId}/timetableSettings`,
      "periods",
    );
    const unsubPeriods = onSnapshot(periodsRef, (periodsSnap) => {
      const periods: PeriodData[] = periodsSnap.exists()
        ? periodsSnap.data().periods
        : [];

      const timetablesQuery = collection(
        firestore,
