...
const escalationTriggerMessage =
  "Understood. I'm escalating your request to a human administrator who will get back to you shortly.";
...
export function AiChat() {
  ...
  const hasAdminReplied = React.useMemo(
  ...
  const sendMessage = async (messageContent: string) => {
    if (!messageContent.trim() || isLoading || !user || !schoolId || !role)
      return;
    ...
    try {
      ...
      if (!currentChatId) {
        ...
        await updateDoc(
          doc(firestore, `schools/${schoolId}/support-chats`, currentChatId),
          {
            messages: newMessages,
            lastMessage: messageContent,
            lastUpdate: serverTimestamp(),
