...
type TimetableCellData = {
  ...
  room: string;
  className?: string;
  clash?: { with: string; message: string } | null;
};
...
function DraggableSubject({ subject }: { subject: DraggableSubjectType }) {
  ...
  return (
    <div ref={setNodeRef} style={style} {...listeners} {...attributes}>
      <div
        className={cn(
          "p-3 rounded-md text-white flex items-center justify-between cursor-grab active:cursor-grabbing",
          subject.color,
        )}
      >
        <div>
...
export function TimetableDisplay({
  ...
  }, [schoolId, selectedItem, view, user]);
  ...
  React.useEffect(() => {
    ...
    if (view === "class") {
      ...
      Object.keys(allTimetables).forEach((classId) => {
        ...
        const className =
          allClasses.find((c) => c.id === classId)?.name || classId;
    ...
    for (const timetableId in allTimetablesData) {
      ...
      if (otherLesson && otherLesson.subject.teacher === subject.teacher) {
        const conflictingClassName =
          allClasses.find((c) => c.id === timetableId)?.name || timetableId;
        return {
  ...
  const handleDragEnd = (event: DragEndEvent) => {
    ...
    const subject = active.data.current?.subject as
  ...
  const handleSavePeriods = async () => {
    ...
    try {
      ...
      toast({
        title: "Periods Saved",
        description:
          "The school day periods have been updated for all timetables.",
      });
    } catch (e) {
      console.error(e);
  ...
  const handleAddNewSubject = async () => {
    if (!newSubjectName || !newSubjectCode || !newSubjectDept || !schoolId) {
      toast({
        ...
        description: "Please fill out all subject details.",
        variant: "destructive",
      });
      return;
    }
    ...
    return (
      <Card>
        <CardHeader>
          ...
          <CardDescription>
            Please add a class before creating a timetable.
          </CardDescription>
        </CardHeader>
        <CardContent>
          <div className="flex h-64 items-center justify-center rounded-lg border-2 border-dashed">
            <p className="text-muted-foreground">
              Go to "Classes & Subjects" to get started.
            </p>
          </div>
        </CardContent>
      </Card>
                  ...
                  <table className="w-full min-w-[800px]">
                    <thead>
                      <tr className="border-b bg-muted/50">
                        <th className="p-2 w-32 font-medium text-muted-foreground border-r text-center">
                          Time
                        </th>
                        {days.map((day) => (
                          <th
                    ...
                    <tbody>
                      {periods.map((period) => (
                        <tr key={period.id} className="border-b">
                          <td className="p-2 font-semibold text-primary text-sm border-r text-center">
                            {period.time}
                          </td>
                          {days.map((day) => {
                            ...
                            const content = (
                              <div
                                className={cn(
                                  "h-full w-full",
                                  cellData?.clash &&
                                    "relative ring-2 ring-destructive ring-inset rounded-md",
                                )}
                              >
                                {cellData?.clash && (
                                ...
                                {period.isBreak ? (
                                  ...
                                  <div
                                    className={cn(
                                      "p-2 rounded-md text-white h-full flex flex-col justify-between cursor-pointer",
                                      cellData.subject.color,
                                    )}
                                  >
                                    <div>
